...
void scrollbar_init(scrollbar_type *scrollbar, unsigned int scroll_position, unsigned int total_elements)
{
    ...
    scrollbar->image_button_scroll_down = (image_button) { 0, 0, SCROLL_BUTTON_SIDE, SCROLL_BUTTON_SIDE, IB_SCROLL,
        0, scrolldown_id, text_scroll, button_none, 1, 1, 1 };
}
...
void scrollbar_draw(scrollbar_type *scrollbar)
{
    if (scrollbar->max_scroll_position > 0 || scrollbar->always_visible) {
        ...
        image_buttons_draw(scrollbar->x, scrollbar->y + scrollbar->height - SCROLL_BUTTON_SIDE,
        ...
        int offset = calc_adjust_with_percentage(
            scrollbar->height - TOTAL_SCROLL_BUTTON_HEIGHT - 2 * scrollbar->dot_padding, pct);
        if (scrollbar->is_dragging_scrollbar_dot) {
            offset = scrollbar->scrollbar_dot_drag_offset;
        }
        int middle_part_id = assets_lookup_image_id(ASSET_UI_SCROLLBAR_MIDDLE);
        image_draw(middle_part_id,
            scrollbar->x + (SCROLL_BUTTON_SIDE - SCROLL_DOT_WIDTH) / 2,
            scrollbar->y + offset + SCROLL_BUTTON_SIDE + scrollbar->dot_padding,
...
static int handle_scrollbar_dot(scrollbar_type *scrollbar, const mouse *m)
{
    ...
    int track_height = scrollbar->height - TOTAL_SCROLL_BUTTON_HEIGHT - 2 * scrollbar->dot_padding;
    if (m->x < scrollbar->x || m->x >= scrollbar->x + SCROLL_BUTTON_SIDE) {
        return 0;
    }
    if (m->y < scrollbar->y + SCROLL_BUTTON_SIDE + scrollbar->dot_padding ||
        m->y > scrollbar->y + scrollbar->height - SCROLL_BUTTON_SIDE - scrollbar->dot_padding) {
        return 0;
    }
    int dot_offset = m->y - scrollbar->y - SCROLL_DOT_HEIGHT / 2 - SCROLL_BUTTON_SIDE;
    if (dot_offset < 0) {
        dot_offset = 0;
    }
...
int scrollbar_handle_mouse(scrollbar_type *scrollbar, const mouse *m, int in_dialog)
{
    ...
    if (scrollbar->touch_drag_state != TOUCH_DRAG_IN_PROGRESS) {
        ...
        if (image_buttons_handle_mouse(m, scrollbar->x, scrollbar->y + scrollbar->height - SCROLL_BUTTON_SIDE,
