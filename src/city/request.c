    ...
    if (request) {
        if (request->resource == RESOURCE_DENARII) {
            if (city_finance_treasury() <= (int) request->amount.requested) {
                //cast to int to ensure correct comparison even if treasury is negative
                return CITY_REQUEST_STATUS_NOT_ENOUGH_RESOURCES;
            }
        } else {
            unsigned int amount = city_resource_get_amount_for_request(request->resource, request->amount.requested);
            // resource can't have negative storage, unlike treasury
